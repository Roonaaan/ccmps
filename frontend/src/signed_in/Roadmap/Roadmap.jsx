...
const Roadmap = () => {
  ...
  const confirmAssessment = async () => {
    ...
    if (confirmPrompt.isConfirmed) {
      ...
      const response = await fetch(`https://ccmps-server-node.vercel.app/api/auth/retry-count?email=${userEmail}&job=${encodeURIComponent(selectedJobTitle)}`, {
        method: 'POST',
      });
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        ...
        const response = await fetch(`https://ccmps-server-node.vercel.app/api/auth/courses?job=${encodeURIComponent(selectedJobTitle)}`);
        const data = await response.json();
