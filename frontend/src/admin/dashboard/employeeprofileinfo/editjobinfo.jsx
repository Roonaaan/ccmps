...
function editjobinfo({ onClose }) {
  ...
  const fetchEmployeeData = async () => {
    try {
      ...
      const response = await axios.get(`http://localhost:8800/api/auth/get-profilejobinfo/${employeeId}`);
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          await axios.delete(`http://localhost:8800/api/auth/delete-profilejobinfo/${editEmployeeId}/${deletedJobId}`);
  ...
  const handleAddJob = async () => {
    try {
      ...
      const response = await axios.get("http://localhost:8800/api/auth/get-next-job-id");
  ...
  const handleSave = async () => {
    try {
      ...
      await axios.post(`http://localhost:8800/api/auth/edit-profilejobinfo/${editEmployeeId}`, {
        company,
        jobTitle,
        skills,
        companyAddress,
  ...
  const handleSaveNewData = async (newJob) => {
    try {
      ...
      await axios.post(`http://localhost:8800/api/auth/add-profilejobinfo/${editEmployeeId}`, {
