...
function editpersonalinfo({ onClose }) {
    ...
    const fetchEmployeeData = async () => {
        try {
            const employeeId = sessionStorage.getItem('editEmployeeId');
            const response = await axios.get(`http://localhost:8800/api/auth/get-profilepersonalinfo/${employeeId}`);
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            const employeeId = sessionStorage.getItem('editEmployeeId');
            await axios.post('http://localhost:8800/api/auth/edit-profilepersonalinfo', { employee_id: employeeId, ...formData });
