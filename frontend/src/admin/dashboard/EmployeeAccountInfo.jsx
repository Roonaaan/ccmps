...
function EmployeeAccountInfo() {
  ...
  const fetchEmployees = async () => {
    try {
      const response = await axios.get('https://ccmps-server-node.vercel.app/api/auth/read-accountinfo');
  ...
  const handleDelete = async (employeeId) => {
    ...
    if (confirmed) {
      try {
        await axios.post('https://ccmps-server-node.vercel.app/api/auth/delete-accountinfo', { employeeId }); // Send employee ID in the request body
