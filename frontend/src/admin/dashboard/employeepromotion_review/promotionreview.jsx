...
function PromotionReview({ onClose }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`http://localhost:8800/api/auth/get-userpromotioninfo/${employeeId}`);
    ...
    const handlePromoteUser = async () => {
        if (userData.promotionStatus === "Eligible for Promotion") {
            try {
                const response = await axios.post(`http://localhost:8800/api/auth/promoteuser/${userData.employee_id}`);
                Swal.fire({
                    icon: 'success',
                    title: 'User Promoted',
