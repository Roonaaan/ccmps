...
function edit({ onClose }) {
  ...
  const fetchEmployeeData = async () => {
    try {
      ...
      const response = await axios.get(`http://localhost:8800/api/auth/get-jobinfo/${employeeId}`);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
        const employeeId = sessionStorage.getItem('editEmployeeId');
        await axios.post('http://localhost:8800/api/auth/edit-jobinfo', { employee_id: employeeId, ...formData });
